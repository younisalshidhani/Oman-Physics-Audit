...
if api_key:
    try:
        ...
        # التعديل الجوهري: استخدام الموديل الذي ظهر في قائمتك المتاحة
        ...
        if test_file:
            if st.button("🚀 بدء المطابقة والتحليل الشامل"):
                with st.spinner("جاري قراءة الملفات الثلاثة والمطابقة العميقة..."):
                    # قراءة نصوص الملفات
                    def get_text(file):
                    ...
                    p_text = get_text(policy_file) if policy_file else "اعتمد على معايير الاختبار القصير (10 درجات)"
                    b_text = get_text(book_file) if book_file else "اعتمد على درس تأثير دوبلر ص 32"

                    prompt = f"""
                    بصفتك خبير جودة تربوي في سلطنة عمان، قم بإجراء مطابقة ثلاثية بين:
                    1. وثيقة التقويم المرفقة: {p_text}
                    2. كتاب الطالب المرفق: {b_text}
                    
                    لتحليل الاختبار التالي: {t_text}
                    
                    المطلوب بدقة:
                    - جدول Markdown بالأعمدة (رقم المفردة، الدرجة، نوع هدف التقويم، نوع الملاحظة، الملاحظات، التعديل المقترح).
                    - توصية نهائية ونسبة مطابقة (%).
                    """
        ...
        # إذا استمر الخطأ، سنعرض رسالة مساعدة
        st.error(f"خطأ في الموديل: {e}")
        st.info("نصيحة: جرب تغيير 'gemini-2.5-flash' إلى 'gemini-1.5-flash' في الكود إذا تكرر الخطأ.")
else:
